    ...
    <div className="min-h-screen pt-28 pb-24 md:pb-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-24">
          ...
          {dict.services.items.map((service, index) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="text-heading font-bold text-foreground mb-3">{service.title}</h3>
                <p className="text-body text-muted-foreground flex-1 mb-8">{service.description}</p>
                <button
                  onClick={openModal}
                  className="w-full py-3 rounded-xl bg-white/5 border border-white/10 text-primary text-body font-bold hover:bg-primary hover:text-white transition-all duration-300"
                >
        ...
        >
          ...
          <h2 className="text-heading font-bold mb-4 relative z-10">{t("services.ctaTitle")}</h2>
          <p className="text-body text-muted-foreground mb-8 max-w-2xl mx-auto relative z-10">{t("services.ctaSub")}</p>
          <button
            onClick={openModal}
            className="relative z-10 inline-flex items-center gap-2 px-8 py-4 rounded-xl bg-foreground text-background font-bold text-body hover:bg-white hover:scale-105 transition-all duration-300"
          >
