...
export default function Auth() {
  ...
  const onLogin = (data: LoginInput) =>
    login.mutate(data, { onSuccess: () => setLocation("/dashboard") });
  const onRegister = (data: RegisterInput) =>
    register.mutate(data, { onSuccess: () => setLocation("/dashboard") });

  const pending = login.isPending || register.isPending;
  const inputClass =
    "w-full pl-12 pr-4 py-3 rounded-xl bg-black/40 border border-white/10 text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all";
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground">
            {isLogin ? "Sign in to book and track your assessments." : "Register to book a site assessment."}
          </p>
        </div>
        ...
        {isLogin ? (
          ...
          <form onSubmit={registerForm.handleSubmit(onRegister)} className="space-y-5" noValidate>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
                <input type="password" placeholder="At least 8 characters" className={inputClass} {...registerForm.register("password")} />
              </div>
            ...
            <button
              ...
              className="w-full py-4 rounded-full bg-primary text-primary-foreground font-medium hover:bg-[#a855f7] transition-colors flex items-center justify-center gap-2 disabled:opacity-70"
            >
              {pending ? <Loader2 className="w-5 h-5 animate-spin" /> : <>Create account <ArrowRight className="w-5 h-5" /></>}
            </button>
          </form>
        )}
